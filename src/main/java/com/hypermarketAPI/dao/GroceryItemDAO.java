...
public class GroceryItemDAO {
  ...
  public GroceryItemDAO() {
    try (InputStream inputStream = GroceryItemDAO.class.getClassLoader()
  ...
  public List<Long> createGroceryItems(final long hypermarketId,
      final List<GroceryItem> groceryItems) throws DAOException {
    ...
    try (Connection conn = DriverManager.getConnection(url, userName, password)) {
      for (final GroceryItem groceryItem : groceryItems) {
        ...
        final int numberInserted = context.insertInto(table("GROCERY_ITEM"),
            ...
            field("GROCERY_ITEM.BRAND"), field("GROCERY_ITEM.EXPIRATION_DATE"))
            .values(hypermarketId,
                groceryItem.getItemName(), groceryItem.getBrand(), groceryItem.getExpirationDate())
            .execute();
        if (numberInserted > 0) {
  ...
  public GroceryItemInstance getGroceryItemById(final long groceryItemId) {
    try (Connection conn = DriverManager.getConnection(url, userName, password)) {
      final DSLContext context = DSL.using(conn, SQLDialect.MYSQL);
      final ResultQuery<Record4<Object, Object, Object, Object>> resultQuery = context
          .select(field("GROCERY_ITEM.HYPERMARKET_ID"),
              field("GROCERY_ITEM.ITEM_NAME"), field("GROCERY_ITEM.BRAND"),
              field("GROCERY_ITEM.EXPIRATION_DATE")).from(table("GROCERY_ITEM"))
          .where(field("GROCERY_ITEM.GROCERY_ITEM_ID").eq(groceryItemId));
  ...
  public List<GroceryItemInstance> getGroceryItemByHypermarketId(final long hypermarketId) {
    try (Connection conn = DriverManager.getConnection(url, userName, password)) {
      final DSLContext context = DSL.using(conn, SQLDialect.MYSQL);
      final ResultQuery<Record5<Object, Object, Object, Object, Object>> resultQuery = context
          .select(field("GROCERY_ITEM.GROCERY_ITEM_ID"), field("GROCERY_ITEM.HYPERMARKET_ID"),
              field("GROCERY_ITEM.ITEM_NAME"), field("GROCERY_ITEM.BRAND"),
              field("GROCERY_ITEM.EXPIRATION_DATE")).from(table("GROCERY_ITEM"))
          .where(field("GROCERY_ITEM.HYPERMARKET_ID").eq(hypermarketId));
      ...
      for (int i = 0; i < result.size(); i++) {
        ...
        final Date expirationDate = (Date) result
            .getValue(i, field("GROCERY_ITEM.EXPIRATION_DATE"));
