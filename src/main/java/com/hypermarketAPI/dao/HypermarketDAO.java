...
public class HypermarketDAO {
  ...
  public HypermarketDAO() {
    ...
    try (InputStream inputStream = HypermarketDAO.class.getClassLoader()
  ...
  public long createHypermarket(Hypermarket hypermarket) throws DAOException {
    try (Connection conn = DriverManager.getConnection(url, userName, password)) {
      ...
      final int numberInserted = context
          .insertInto(table("HYPERMARKET"), field("HYPERMARKET.NAME"), field("HYPERMARKET.ADDRESS"))
  ...
  public HypermarketInstance getHypermarketById(long id) throws DAOException {
    try (Connection conn = DriverManager.getConnection(url, userName, password)) {
      ...
      final ResultQuery<Record3<Object, Object, Object>> query = context
          .select(field("HYPERMARKET.HYPERMARKET_ID"), field("HYPERMARKET.NAME"),
              field("HYPERMARKET.ADDRESS"))
  ...
  public void deleteHypermarketById(final long hypermarketId) throws DAOException {
    try (Connection conn = DriverManager.getConnection(url, userName, password)) {
      final DSLContext context = DSL.using(conn, SQLDialect.MYSQL);
      final Query query = context.delete(table("HYPERMARKET"))
          .where(field("HYPERMARKET.HYPERMARKET_ID").eq(hypermarketId));
